...
class IngredientService {
  ...
  Future<List<Ingredient>> getIngredients(Recipe recipe) async {
    if (!AppComponent.DEBUG_MODE && recipe.link == null) {
      return [];
    }
    ...
    if (!AppComponent.DEBUG_MODE) {
      final String requestURL = _SERVICE_URL.replaceAll('%', Uri.encodeFull(recipe.link));
      json = await HttpRequest.getString(requestURL);
    }
